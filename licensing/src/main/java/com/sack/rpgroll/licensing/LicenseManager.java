...
public class LicenseManager {
    ...
    /**
     * Copia {@code license/license.yml} del jar a la carpeta del plugin la
     * primera vez, sin sobreescribir lo que ya exista.
     * <p>
     * Se hace acá en vez de reusar el ResourceCopier de {@code :common} a
     * propósito: este módulo no depende de ningún otro del ecosistema, y así
     * SackResourcePack —que es independiente de RPGRoll— también puede
     * verificar su licencia sin arrastrar {@code :common}.
     */
    private File loadLicenseFile() {

        File destination = new File(plugin.getDataFolder(), "license.yml");

        if (destination.exists()) {
            return destination;
        }

        try (InputStream bundled = plugin.getResource("license/license.yml")) {

            if (bundled == null) {
                plugin.getLogger().severe("✘ El jar no trae license/license.yml — build mal empaquetada.");
                return destination;
            }

            Files.createDirectories(destination.getParentFile().toPath());
            Files.copy(bundled, destination.toPath(), StandardCopyOption.REPLACE_EXISTING);

        } catch (IOException e) {
            plugin.getLogger().severe("✘ No se pudo crear license.yml: " + e.getMessage());
        }

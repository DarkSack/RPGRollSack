...
public class AnvilRecipeDefinitionWriter {

    private final File folder;

    public AnvilRecipeDefinitionWriter(File dataFolder) {
        this.folder = new File(dataFolder, "anvil-recipes");
    }
    ...
    public void save(AnvilRecipeDefinition recipe) {
        ...
        config.set("repair-cost-levels", recipe.repairCostLevels());

        writeIngredient(config, "base", recipe.baseIngredient());
        writeIngredient(config, "addition", recipe.additionIngredient());
        ...
        try {
            ...
            throw new RuntimeException("No se pudo guardar la receta de yunque " + recipe.id(), e);
        }
    }

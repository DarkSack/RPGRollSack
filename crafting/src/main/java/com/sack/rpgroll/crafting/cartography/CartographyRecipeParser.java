...
public class CartographyRecipeParser implements ContentParser<CartographyRecipeDefinition> {

    @Override
    public CartographyRecipeDefinition parse(YamlConfiguration config) throws Exception {
        ...
        IngredientSpec map = parseIngredient(config.getConfigurationSection("map"), id, "map");
        IngredientSpec item = parseIngredient(config.getConfigurationSection("item"), id, "item");
        RecipeResult result = parseResult(config.getConfigurationSection("result"), id);

        return new CartographyRecipeDefinition(id, map, item, result, parseConditions(config.getMapList("conditions"), id));
    }

    private IngredientSpec parseIngredient(ConfigurationSection section, String recipeId, String field) {

        if (section == null) {
            throw new IllegalArgumentException("receta de cartografía '" + recipeId + "' sin sección obligatoria '" + field + "'");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException("receta de cartografía '" + recipeId + "' tiene " + field + ".type inválido");
        }
    ...
    private RecipeResult parseResult(ConfigurationSection section, String recipeId) {
        ...
        if (section == null) {
            throw new IllegalArgumentException("receta de cartografía '" + recipeId + "' sin sección obligatoria 'result'");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException("receta de cartografía '" + recipeId + "' tiene result.type inválido");
        }
    ...
    private List<RecipeCondition> parseConditions(List<?> raw, String recipeId) {
        ...
        for (Object entry : raw) {
            ...
            try {
                ...
                throw new IllegalArgumentException("receta de cartografía '" + recipeId
                        + "' tiene una condición con type inválido: " + map.get("type"));
            }
